 ...
 * - Index generation with rich project overview
 */
...
function generateProjectOverview(entries: KbEntry[]): string {
	...
	// Build overview — now using rich descriptions from entries
	...
	// Synthesize a "what this project does" section from entry descriptions
	const moduleEntries = entries.filter(e => e.frontmatter.category === "module");
	if (moduleEntries.length > 0) {
		parts.push("");
		parts.push("**Modules**:");
		for (const entry of moduleEntries.slice(0, 8)) {
			...
			if (desc && desc.length > 5) {
				...
				parts.push(`- **${title}**: ${fileCount} file(s)`);
			}
		}
	}

	// Summarize what each category covers
	const nonModuleCategories = categories.filter(c => c !== "module");
	if (nonModuleCategories.length > 0) {
		parts.push("");
		parts.push("**Other areas**:");
		for (const cat of nonModuleCategories) {
			...
			// Collect descriptions from entries in this category
			const descriptions = catEntries
				.map(e => e.frontmatter.description)
				.filter(d => d && d.length > 10 && !d.startsWith(`${cat} —`))
				.slice(0, 3);

			if (descriptions.length > 0) {
				parts.push(`- **${label}** (${fileCount} files): ${descriptions.join("; ")}`);
			} else {
				parts.push(`- **${label}**: ${fileCount} file(s) in ${catEntries.length} entry(ies)`);
			}
		}
	}

	parts.push("");
	parts.push("> 💡 Use `kb_query` to search entries by keyword, category, or tag. Each entry contains extracted knowledge about purpose, responsibilities, API surface, and more.");
...
export async function generateIndex(kbDir: string, entries: KbEntry[]): Promise<void> {
	...
	// Project overview section — now with rich descriptions
	...
	for (const [cat, catEntries] of byCategory) {
		...
		for (const entry of catEntries) {
			...
			const fileCount = entry.frontmatter.sourceFiles.length;
			// Truncate long descriptions in the index
			const shortDesc = entryDesc.length > 120 ? entryDesc.slice(0, 117) + "..." : entryDesc;
			lines.push(`- [${entry.frontmatter.title}](./${link}) — ${shortDesc} (${fileCount} files)`);
		}
		lines.push("");
	}
